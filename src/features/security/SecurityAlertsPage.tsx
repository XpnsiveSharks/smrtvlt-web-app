  ...
  return (
    <section className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <header className="flex flex-wrap items-center justify-between gap-3">
        ...
        <Button variant="secondary" onClick={() => void refetch()} isLoading={loading} className="border-white/5 hover:bg-white/5 gap-2 group">
      ...
      {!loading && !error && data && (
        <div className="space-y-6 relative">
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 delay-100 fill-mode-both">
            <Card className="relative overflow-hidden group border border-white/5 transition-colors duration-500 shadow-2xl">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10 items-center">
                <div className="md:border-r md:border-white/5 pr-8">
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 delay-300 fill-mode-both">
            <Card className="min-h-[200px] border border-white/5 shadow-xl">
              ...
              {data.active_alerts.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 bg-app-bg/20 rounded-xl border border-dashed border-white/5">
                ...
                <div className="space-y-4">
                  {data.active_alerts.map((alert: SecurityAlert, index: number) => (
                    <div
                      key={index}
                      className="group relative rounded-xl border border-white/5 bg-app-bg/30 p-5 hover:border-brand/20 transition-all hover:bg-app-bg/50 shadow-inner"
                    >
                      ...
                      <div className="mt-5 flex flex-wrap items-center gap-x-8 gap-y-2 text-[10px] font-bold uppercase tracking-[0.15em] text-app-muted opacity-50">
                        <span className="flex items-center gap-2 group-hover:text-app-text transition-colors">
                          Count <span className="font-mono text-app-text bg-white/5 px-2 py-0.5 rounded leading-none border border-white/5">{alert.count}</span>
                        </span>
                        <span className="flex items-center gap-2 group-hover:text-app-text transition-colors">
                          Threshold <span className="font-mono text-app-text bg-white/5 px-2 py-0.5 rounded leading-none border border-white/5">{alert.threshold}</span>
                        </span>
                        <span className="flex items-center gap-2 group-hover:text-app-text transition-colors">
                          Window <span className="text-app-text bg-white/5 px-2 py-0.5 rounded leading-none border border-white/5 italic">{alert.window}</span>
                        </span>
                      </div>
                    </div>
                  ))}
...
function MetricTile({ label, value, accentColor }: { label: string; value: number; accentColor: string }) {
  return (
    <div className={`bg-app-surface rounded-lg p-5 border border-white/5 border-l-2 ${accentColor} shadow-sm group hover:border-white/10 transition-all relative overflow-hidden`}>
      <div className="absolute top-0 right-0 p-2 opacity-5 pointer-events-none">
